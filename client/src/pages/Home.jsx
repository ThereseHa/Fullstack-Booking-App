...
export default function ResponsiveDateTimePickers() {
  ...
  const fetchBookedTimes = async () => {
    try {
      ...
      const bookedTimes = response.data.map((booking) => booking.booking_date);
      setBookedTimes(bookedTimes);
    } catch (error) {
      console.error(error);
    }
    ...
    <Box p={2} sx={{ display: "flex", justifyContent: "center" }}>
      <Grid container spacing={2} sx={{ justifyContent: "center" }}>
        <Grid item xs={12} sx={{ textAlign: "center" }}>
          <Box mt={2}>
            <Typography variant="h4">Välj Datum</Typography>
          ...
          {availableTimes.length > 0 ? (
            availableTimes.map((time) => {
              const isBooked = bookedTimes.some(
                (bookedTime) => format(new Date(bookedTime), "HH:mm") === time
              );
              ...
              return (
                <Button
                  ...
                  sx={{
                    ...
                    backgroundColor: isBooked ? red[500] : "primary.main",
                    "&:disabled": {
        ...
        <Grid item xs={12}>
          ...
          {bookedTimes.map((time) => (
            <Box key={time} display="flex" alignItems="center" sx={{ mt: 1 }}>
              <Typography>
